...
class Billing extends Controller
{
	...
	function firm_edit()
	{
        ...
        #dup sign
        $data['firm_dup_sign'] = $this->db->query("select dup_sign from industry.firm where id =  {$this->uri->segment(3)}")->row()->dup_sign;

        $data['dup_sign'] = $this->db->query(
            "select
              firm.id,
              firm.name,
              firm.dogovor
            ...
            where industry.firm_group.id in (
              select firm_group.id
              from industry.firm_group
                join industry.firm_subgroup
                  on firm_subgroup.group_id = firm_group.id
                join industry.firm on firm.subgroup_id = firm_subgroup.id
              where firm.id in
                    (
                      select firm.dup_sign
                      from industry.firm
                      where firm.id = {$this->uri->segment(3)}
                    )
            )
    ...
    function schetfactura()
	{
		...
		$data['data_nakl']=$_POST['data_nakl'];
			...
			$data['dog_real2']=$_POST['dog_real2']	;		
	...
	{		
		...
		insert into industry.oplata 
			...
			where industry.firm_id_by_nomer1c(nomer1c) is not null and industry.schet_id_by_name(schet) is not null AND oplata_buf.vo <> 4"
			);
			...
			foreach ($nach->result() as $n)
			{
				dbase_add_record($db2,
					array (
						$this->d2($n->DATA),
						$n->NAIM,
						$n->DOG,$n->NACH,$n->NACH_SUM,$n->NACH_NDS, $n->KVT, $n->NOMERSCHET, $n->KONDATA, $n->NPENI
					));
			}
...
where firm_id={$this->uri->segment(3)} and data_finish is null";
    ...
    public function fine_2_re()
    {
		...
		public function pre_dup7re()
    {
        $data['periods'] = $this->db->get("industry.selected_period");
        $data['ture'] = $this->db->get("industry.ture");
        $this->left();
        $this->load->view("other_reports/dup7re/pre", $data);
        $this->load->view("right");
    }


    public function dup7re()
    {
        $sql = 'select * from industry.period where id=' . $_POST['period_id'];
        $data['period'] = $this->db->query($sql)->row();
        $sql = "SELECT * FROM industry.org_info";
        ...
        switch ($_POST['firm_type']) {
            case 'debet':
                $this->db->where('debet_end >', '0');
                break;
            case 'kredit':
                $this->db->where('kredit_end >', '0');
                break;
            default:
                break;
        }
        $this->db->where('ture_id', $_POST['ture_id']);
        $this->db->where('period_id', $_POST['period_id']);
        $data['report'] = $this->db->get("industry.dup7re");

        $this->load->view("other_reports/dup7re/report", $data);
    }
   	...
   	public function export_rekvizit_schet()
    {
        ...
        if ($db) {
            ...
            if (!copy("c:/oplata/rschet.dbf", "c:/oplata/keller{$month}.dbf")) {
    ...
    public function migration()
    {
        ...
        $this->export_to_excel("other_reports/migration", $data,"ленинградский_пэу");
